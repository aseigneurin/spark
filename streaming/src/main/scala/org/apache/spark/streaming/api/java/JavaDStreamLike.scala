...
trait JavaDStreamLike[T, This <: JavaDStreamLike[T, This, R], R <: JavaRDDLike[T, R]]
   ...
   * 'this' DStream will be registered as an output stream and therefore materialized.
   *
   * @deprecated  As of release 1.5.0, replaced by foreachRDD(VoidFunction[R])
   */
  @Deprecated
  def foreachRDD(foreachFunc: JFunction[R, Void]) {
    val f: JVoidFunction[R] = (r: R) => foreachFunc.call(r)
    foreachRDD(f)
  }
  ...
  def foreachRDD(foreachFunc: JFunction2[R, Time, Void]) {
    val f: JVoidFunction2[R, Time] = (r: R, time: Time) => foreachFunc.call(r, time)
    foreachRDD(f)
  }
